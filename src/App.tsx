...
type Screen = 'home' | 'section-select' | 'intro' | 'test' | 'result' | 'vocabulary' | 'voice-settings' | 'history' | 'exam-data'
...
function isAppNavigationState(value: unknown): value is AppNavigationState {
  ...
  return state.focusEnglishLab === true && typeof state.screen === 'string' && ['home', 'section-select', 'intro', 'test', 'result', 'vocabulary', 'voice-settings', 'history', 'exam-data'].includes(state.screen)
}
...
function Intro({ items, mode, onBack, onStart }: { items: BaseItem[]; mode: PracticeMode; onBack: () => void; onStart: () => void }) {
  ...
  useEffect(() => {
    ...
    prepareExamTTS(examAudioTexts, profile.id, (message) => { if (active) setPreparation((current) => ({ ...current, message })) })
      .then((result) => {
        if (!active) return
        setPreparation({ phase: result.fallback ? 'fallback' : result.engine === 'system' ? 'system' : 'ready', message: result.fallback ? 'AI 엔진 대신 기기 영어 음성이 준비됐습니다.' : result.engine === 'system' ? '선택한 기기 영어 음성이 준비됐습니다. 첫 듣기 문항부터 바로 재생됩니다.' : '로컬 AI 서버 준비 완료 · 시험 음성은 자동으로 캐시됩니다.' })
      })
