...
function Question({ item, answer, locked = false, showFeedback = false, onAnswer, onAudioState, onCoachState, coachEnabled = true, audioAlreadyPlayedForGroup = false }: { item: BaseItem; answer: Answer | undefined; locked?: boolean; showFeedback?: boolean; onAnswer: (answer: Answer) => void; onAudioState?: (active: boolean) => void; onCoachState?: (active: boolean) => void; coachEnabled?: boolean; audioAlreadyPlayedForGroup?: boolean }) {
  ...
  if (item.kind === 'sentence-build') {
    ...
    const sentenceCorrect = isCorrect(item, chosen)
    return <div className="single-column"><div className="question-title"><span>{item.title}</span><ContextLabel item={item} /><h1>{item.instruction}</h1></div><p className="conversation">{item.prompt}</p><div className={`sentence-target${showFeedback ? sentenceCorrect ? ' sentence-target--correct' : ' sentence-target--wrong' : ''}`}><strong>{item.starter}</strong><div className="sentence-slots">{Array.from({ length: slotCount }, (_, index) => chosen[index] ? <button disabled={locked} onClick={() => onAnswer(chosen.filter((_, chosenIndex) => chosenIndex !== index))} key={`${chosen[index]}-${index}`}>{chosen[index]}</button> : <span aria-hidden="true" key={`empty-${index}`} />)}</div></div>{showFeedback && <InlineAnswerNote correct={sentenceCorrect} answer={sentenceCorrect ? undefined : `${item.starter} ${String(item.answer).split('|').join(' ')}`} explanation={item.explanation} />}<div className="word-bank" aria-label="섞인 단어와 구">{available.map((word, index) => <button disabled={locked || chosen.length >= slotCount} onClick={() => onAnswer([...chosen, word])} key={`${word}-${index}`}>{word}</button>)}</div></div>
  }
...
function InlineAnswerNote({ answer, explanation, correct = false }: { answer?: string; explanation?: string; correct?: boolean }) {
  return <div className={correct ? 'inline-answer-note inline-answer-note--correct' : 'inline-answer-note'} role="status"><span>{correct ? '정답입니다' : answer ? '정답' : '해설'}</span>{answer && <strong>{answer}</strong>}{explanation && <p><b>해설</b>{explanation}</p>}</div>
}
