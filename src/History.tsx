...
export default function History({ onBack, initialSessionId = null }: { onBack: () => void; initialSessionId?: string | null }) {
  ...
  return <div className="history-page"><header><Brand /><button className="text-button" onClick={onBack}><ArrowIcon direction="left" /> 홈으로</button></header><main><div className="history-hero"><div><span>Learning history</span><h1>학습 기록</h1><p>완료한 시험의 답안과 해설을 다시 보고, 해당 문항을 앞으로의 랜덤 출제에 넣을지 결정할 수 있습니다.</p></div><dl><div><dt>완료한 시험</dt><dd>{history.length}</dd></div><div><dt>랜덤 제외 문항</dt><dd>{excludedItems}</dd></div></dl></div>{history.length === 0 ? <div className="history-empty"><strong>완료한 시험이 없습니다.</strong><span>시험을 끝까지 마치면 기록과 답안이 여기에 저장됩니다.</span></div> : <section className="history-cards" aria-label="완료한 학습 기록">{history.map((session) => {
    const items = sessionItems(session)
    const stats = getSessionStats(items, session)
