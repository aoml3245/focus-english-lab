...
export default function PersonalVocabularyWorkspace({ vocabulary, onLibraryChanged }: Props) {
  ...
  return <section className="personal-vocabulary" aria-label="내 단어장 관리">
    ...
    <div className="personal-vocabulary__backup"><div><strong>iCloud 이동용 백업</strong><p>내보낸 JSON 파일을 데스크톱 또는 iCloud Drive에 두고, iPhone의 파일 앱에서 가져오면 기존 기록과 안전하게 병합됩니다.</p></div><div><button className="button button--secondary" onClick={downloadPersonalVocabularyBackup}>내 단어장 내보내기</button><button className="button button--secondary" onClick={() => importRef.current?.click()}>내 단어장 가져오기</button><input ref={importRef} type="file" accept="application/json,.json" hidden onChange={(event) => importBackup(event.target.files?.[0])} /></div></div>
