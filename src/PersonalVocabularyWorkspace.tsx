...
export default function PersonalVocabularyWorkspace({ vocabulary, onLibraryChanged }: Props) {
  ...
  const saveInput = async () => {
    ...
    try {
      ...
      setInput('')
      setSelectedWords({})
      setStatus(`${entries.length}개 단어를 오늘 학습 목록에 저장했습니다.`)
  ...
  return <section className="personal-vocabulary" aria-label="내 단어장 관리">
    <div className="personal-vocabulary__head"><div><span>MY DAILY VOCABULARY</span><h2>오늘 외울 단어 넣기</h2><p>한 줄에 <code>영단어, 한국어 뜻</code> 형식으로 여러 개를 붙여넣으세요. 입력한 단어는 두 방향을 모두 맞힌 뒤에만 내 단어장에 들어갑니다.</p></div><dl><div><dt>학습한 단어</dt><dd>{trackedWords}</dd></div><div><dt>누적 오답</dt><dd>{totalIncorrect}</dd></div></dl></div>
    {completed && <div className="personal-vocabulary__complete" role="status"><strong>{completed.words}개 단어를 내 단어장에 추가했습니다</strong><span>{completed.attempts}회 시도 · 오답 {completed.incorrect}회까지 저장했습니다.</span></div>}
    {batch ? <div className="personal-vocabulary__pending"><div><span>학습 중인 단어</span><strong>{batch.entries.length}개 · {batch.totalAttempts}회 시도</strong><p>두 방향 시험을 모두 맞히면 자동으로 내 단어장에 추가됩니다.</p></div><button className="button button--primary button--large" onClick={() => setTraining(true)}>{batch.totalAttempts ? '이어 풀기' : '암기 확인 시작'} <ArrowIcon /></button></div> : <div className="personal-vocabulary__input"><label><span>단어 여러 개 붙여넣기</span><textarea value={input} onChange={(event) => setInput(event.target.value)} placeholder={'abandon, 버리다; 포기하다\ncoherent, 일관성 있는\nsubsequent, 그 다음의'} /></label>{matchRows.length > 0 && <section className="word-match-list" aria-label="기존 단어장 검색 결과">{matchRows.map((row) => <WordMatchRow key={row.word} word={row.word} meaningKo={row.meaningKo} matches={row.matches} selectedWord={selectedWords[row.word] || row.matches.find((match) => match.kind === 'exact')?.entry.word || row.word} onSelect={(selectedWord) => setSelectedWords((current) => ({ ...current, [row.word]: normalizeWord(selectedWord) }))} />)}</section>}<div><span>{parsed.length ? `${parsed.length}개 인식됨 · 기존 단어 후보를 확인해 주세요.` : '영단어와 뜻이 모두 있는 줄만 저장됩니다.'}</span><button className="button button--primary button--large" disabled={!parsed.length} onClick={saveInput}>오늘 단어 저장 <ArrowIcon /></button></div></div>}
